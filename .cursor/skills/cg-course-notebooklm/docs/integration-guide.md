...
| 轮次 | 目标 | 必做动作 | 输出检查 |
|------|------|----------|----------|
| Draft 0 | 基础框架 | 按知识图谱搭章节层次，先写清全景、核心问题、术语表；来源引用先贴近对应章节占位，不做末尾大表 | 结构完整但可暂缺细节 |
| Draft 1 | 基础补充 | 回到 raw 补齐每节基础解释、来源、公式/坐标含义、管线位置；首次出现专业术语必须解释并纳入术语表 | 每节能独立读懂 |
| Draft 2 | 重难点深挖 | 对核心/难点补直观解释、例题、形象比喻、易混对比、常见错误 | 重点不只停留在定义 |
| Draft 3 | 可视化与串联 | 引入合适 Mermaid/ASCII 图；补章间承接、前后周/Project 桥接 | 知识点之间连成线 |
| Review | 自审迭代 | 按 `checklist.md` 找缺口；检查术语首现、英文对照、就近引用、非代码数学表达 LaTeX 渲染、Markdown 表格竖线、最终指南无采集计划；必要时追加 `supplement-*` raw | 进入用户 Review 前已修过一轮 |
...
- 期末为英语考试。专业术语在术语表和正文首次出现时采用 `中文(English)` 格式，例如 `齐次坐标(Homogeneous Coordinates)`。
- 英文缩写采用 `Abbr(Full English Name，中文对照)` 格式，例如 `NDC(Normalized Device Coordinates，规范化设备坐标)`。
- 正文首次出现的新术语必须用一句话解释，不允许只出现名词；首现术语必须同步进入 `## 0. 术语表` 或该 Part 的局部术语块。
- Draft/Review 阶段要专门搜索未解释术语，尤其是算法名、缩写、渲染管线阶段、坐标空间和矩阵名。
...
- 表格单元格中出现竖线时必须转义或改写。优先把绝对值条件写成 `abs(m) <= 1`，不要在表格中直接使用竖线绝对值写法。
- 复杂公式、矩阵、分段条件优先放到表格外段落中，避免破坏 Markdown 渲染。
- 用户 Review 前必须轻量检查表格行列数、未转义 `|`、术语表渲染和 Mermaid 渲染。

### 3.6 数学表达渲染规范

- 公式、矩阵、参数方程、齐次坐标点/向量表示等非代码数学内容，优先写成 Markdown LaTeX：短表达式用 inline `$...$`，矩阵、分段条件和多行推导用 block `$$...$$`。
- 只有真实代码、伪代码、命令、日志或需要等宽保留的 ASCII 图才使用 fenced code block；不要用 `text` 代码块承载普通公式、矩阵或参数方程。
- Review 时专门搜索行内代码和 `text` 代码块，检查其中是否混入本应 LaTeX 渲染的数学表达。
