...
- [ ] 关键结论可追溯到来源，且核心章节附近有简短 `参考 raw` 引用块
- [ ] 最终指南没有末尾集中大“资料索引”表；完整映射已留在知识图谱或 review/iteration 文档
- [ ] 最终指南没有 `Step 4 补充采集说明`、补采计划或 manifest 计划；这些内容已移到 raw/review/iteration 文档
- [ ] Markdown 表格安全：表格单元格内没有未转义竖线，例如将 `|m|` 改写为 `abs(m)`
- [ ] 课纲偏差已标注
